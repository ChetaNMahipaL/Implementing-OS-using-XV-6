...
found:
  ...
  // Intialising values for sigalarm and sigreturn
  p->sig_status=0;
  p->interval=0;
  p->curr_ticks=0;
  p->sig_handler=0;
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p != 0 && p->state == RUNNABLE && (ticks-(p->entry_time)) >= 32) // https://lass.cs.umass.edu/~shenoy/courses/fall12/labs/2/
      {
        if(p->check_in == 1)
        {
...
void procdump(void)
{
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    ...
    if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
    printf("%d %s %s %d", p->pid, state, p->name, p->level);
    printf("\n");
  }
}
...
int waitx(uint64 addr, uint *wtime, uint *rtime)
{
  ...
  //   {
  //     if(mlfq[y][k].qtime )
  //     printf("%d, ",mlfq[y][k].level);
  //   }
  //   printf("\n");
  //   for(int k=0;k<300;k++)
  //   {
  //     if(mlfq[y][k].qtime )
  //     printf("%d, ",mlfq[y][k].qtime);
  //   }
  //   printf("\n");
  //   printf("%d\n", y);
  // }
...
void update_time()
{
  ...
  //   {
  //     acquire(&p->lock);
  //     mlfq[p->pid-9][ticks].qtime = ticks;
  //     mlfq[p->pid-9][ticks].level = p->level;
  //     release(&p->lock);
  //   }
  // }
}
