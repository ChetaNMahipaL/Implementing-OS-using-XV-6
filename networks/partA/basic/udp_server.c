...
int main() {
    ...
    int recv_data=recvfrom(sfd, client_response, 1024, 0, (struct sockaddr*)&client, &addr_size);
    if(recv_data < 0)
    {
    ...
    int send_sig=sendto(sfd,send_msg,strlen(send_msg),0,(struct sockaddr*)&client, sizeof(client));
    if( send_sig == (-1))
    {
